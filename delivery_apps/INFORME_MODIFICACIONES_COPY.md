...
### 32. Redirección de Enlaces Cortos vía Backend (HTTP 302)
*   **Cambio:** Se eliminó la lógica de redirección basada en el Frontend (Flet) y se trasladó íntegramente al Backend (FastAPI).
*   **Razón:** En compilaciones Web (`flet build web`), el frontend es estático y no puede realizar redirecciones HTTP eficientes sin cargar toda la app.
*   **Solución:** Se creó el endpoint `/r/{tenant_id}/{code}` en la API. Ahora, al acceder a una URL como `.../r/dona_soco/apk`, el servidor responde instantáneamente con un código **302 Redirect**, enviando al usuario directamente a la URL de destino (ej: GitHub) sin pasar por la aplicación.

### 33. Seguridad de Grado Industrial (Dual Auth: API_KEY + JWT)
*   **Problema:** Exponer la clave maestra en el código de GitHub es un riesgo, pero ocultarla rompía el panel administrativo en la web por la falta de variables de entorno en el navegador.
*   **Solución:**
    1.  **JWT (JSON Web Tokens):** Se implementó un sistema de tokens para el Panel Administrativo. Al loguearse, el servidor entrega un "pase temporal" firmado digitalmente.
    2.  **API_KEY:** Se mantuvo la clave secreta maestra exclusivamente para la comunicación entre servicios y la herramienta CLI (`db_admin.py`).
    3.  **Aislamiento:** El backend ahora valida que el token JWT pertenezca específicamente al `tenant_id` solicitado, impidiendo que un negocio acceda a los datos de otro.

### 34. Estabilización de Acciones Administrativas en Web
*   **Problema:** Las acciones en el panel (editar menú, cambiar configuración) fallaban con error 401 tras ocultar las claves.
*   **Solución:** Se actualizó `database.py` y todas las vistas administrativas para gestionar el ciclo de vida del token JWT. Ahora, el objeto `page` de Flet se utiliza para almacenar y recuperar el token de sesión de forma segura, inyectándolo en los encabezados `Authorization: Bearer` de cada petición administrativa.