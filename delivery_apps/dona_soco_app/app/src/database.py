...
def actualizar_platillo(platillo_id, nombre, descripcion, precio, imagen, descuento=0, is_configurable=0, is_configurable_salsa=0, piezas=1, grupos_opciones_ids="[]", printer_target="cocina", categoria_id=None):
    ...
    try:
        r = httpx.put(f"{API_URL}/menu/{platillo_id}", json=data, headers=HEADERS)
...
def eliminar_platillo(platillo_id):
    try:
        r = httpx.delete(f"{API_URL}/menu/{platillo_id}", headers=HEADERS)
        return r.status_code == 200
    except Exception as e:
...
def actualizar_visibilidad_platillo(platillo_id, is_active):
    try:
        r = httpx.put(f"{API_URL}/menu/{platillo_id}/visibilidad", params={"is_active": is_active}, headers=HEADERS)
        return r.status_code == 200
    except Exception as e:
...
def ocultar_todos_los_platillos():
    try:
        r = httpx.put(f"{API_URL}/admin/menu/visibilidad-global", params={"is_active": 0}, headers=HEADERS)
        return r.status_code == 200
    except Exception as e:
...
def mostrar_todos_los_platillos():
    try:
        r = httpx.put(f"{API_URL}/admin/menu/visibilidad-global", params={"is_active": 1}, headers=HEADERS)
        return r.status_code == 200
    except Exception as e:
...
def update_configuracion(horario, codigos_postales, metodos_pago_activos=None, tipos_tarjeta=None, contactos=None, guisos_disponibles=None, salsas_disponibles=None, costo_envio=20.0, categorias_disponibles=None):
    ...
    try:
        r = httpx.put(f"{API_URL}/configuracion", json=data, headers=HEADERS)
        return r.status_code == 200
    except Exception as e:
...
def obtener_pedido_por_codigo(telefono, codigo):
    try:
        ...
        if response.status_code == 200:
            return _formatear_pedido(response.json())
        return None
    except Exception as e:
        return None

def obtener_pedidos(limit=100, offset=0, search_term=None):
    ...
    try:
        ...
        if response.status_code == 200:
            return [_formatear_pedido(p) for p in response.json()]
        return []
    except Exception as e:
        return []

def obtener_total_pedidos(search_term=None):
    # Traemos una lista amplia para contar (simplificado para esta fase)
    pedidos = obtener_pedidos(limit=1000, search_term=search_term)
    return len(pedidos)

def obtener_pedidos_sin_paginacion(search_term=None):
    return obtener_pedidos(limit=5000, search_term=search_term)
...
def obtener_datos_exportacion(search_term=None):
    try:
        ...
        for o in pedidos:
            for d in o["detalles"]:
                flat_data.append({
                    "orden_id": o["id"], "codigo_seguimiento": o["codigo_seguimiento"],
                    "fecha": o["fecha"], "nombre_cliente": o["nombre_cliente"],
                    "telefono": o["telefono"], "direccion": o["direccion"],
                    "referencias": o["referencias"], "estado": o["estado"],
                    "metodo_pago": o["metodo_pago"], "paga_con": o["paga_con"],
                    "total_orden": o["total"], "motivo_cancelacion": o["motivo_cancelacion"],
...
def actualizar_estado_pedido(orden_id, nuevo_estado, motivo=None):
    ...
    try:
        r = httpx.put(f"{API_URL}/pedidos/{orden_id}/estado", params=params, headers=HEADERS)
        return r.status_code == 200
    except Exception as e:
        return False
...
def actualizar_pago_pedido(orden_id, metodo_pago, paga_con):
    ...
    try:
        r = httpx.put(f"{API_URL}/pedidos/{orden_id}/pago", json=data, headers=HEADERS)
        return r.status_code == 200
    except Exception as e:
        return False
