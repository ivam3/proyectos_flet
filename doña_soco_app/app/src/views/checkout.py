...
def create_checkout_view(page: ft.Page, show_snackbar):
    ...
    nombre_field = ft.TextField(label="Nombre completo", autofocus=True)
    telefono_field = ft.TextField(label="Teléfono de contacto", keyboard_type=ft.KeyboardType.PHONE)
    direccion_field = ft.TextField(label="Dirección completa", multiline=True, max_lines=3)
    referencias_field = ft.TextField(label="Referencias (opcional)", multiline=True, max_lines=2)
    ...
    def confirmar_pedido(e):
        ...
        if exito:
            dialog.title = ft.Text("Pedido registrado ✅")
            ...
            dialog.content = ft.Text("Ocurrió un error al guardar el pedido. Intenta nuevamente.")
    ...
    return ft.Column(
        [
            ...
            referencias_field,
            ft.Divider(),
            ft.Text(f"Total a pagar: ${total:.2f}", size=20, weight="bold"),
