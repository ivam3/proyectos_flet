...
def configuracion_view(page: ft.Page):
    ...
    def cargar_datos():
        ...
        horario_field.value = config["horario"] or ""
        codigos_postales_field.value = config["codigos_postales"] or ""
        ...
        pagos = safe_json_load(config["metodos_pago_activos"], {})
        ...
        tarjetas = safe_json_load(config["tipos_tarjeta"], [])
        ...
        contactos = safe_json_load(config["contactos"], {})
        ...
        for name, val in safe_json_load(config.get("guisos_disponibles"), {}).items():
            chk = ft.Checkbox(
                label=name,
                value=val,
                label_style=ft.TextStyle(color=ft.Colors.BLACK),
        ...
        for name, val in safe_json_load(config.get("salsas_disponibles"), {}).items():
            chk = ft.Checkbox(
                label=name,
                value=val,
                label_style=ft.TextStyle(color=ft.Colors.BLACK),
